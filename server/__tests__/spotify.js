...
const TRACK_INFO = {
  ...
  link: 'https://open.spotify.com/track/11dFghVXANMlKmJXsNCbNl',
  provider: 'spotify',
};

const TRACK_QUERY_STRING = 'Carly%20Rae%20Jepsen%20Cut%20To%20The%20Feeling';

describe('Test Spotify', () => {
  test('_formatQueryString', async () => {
    const Spotify = new spotify();

    const query = { artist: TRACK_INFO.artist, track: TRACK_INFO.track };
    const queryString = Spotify._formatQueryString(query);
    expect(queryString).toBe(TRACK_QUERY_STRING);
  });

  test('_getTrackInfo', async () => {
    const Spotify = new spotify();

    const queryString = Spotify._getTrackInfo(TRACK);
    expect(queryString).toEqual(TRACK_INFO);
  });

  test('_getArtistnfo', async () => {});

  test('_getAlbumInfo', async () => {});
});
